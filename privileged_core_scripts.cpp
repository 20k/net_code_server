...
duk_ret_t scripts__info(priv_context& priv_ctx, duk_context* ctx, int sl)
{
    ...
    if(duk_is_string(ctx, -1))
        script = duk_safe_to_std_string(ctx, -1);
    else if(duk_is_object(ctx, -1))
        script = duk_safe_get_prop_string(ctx, -1, "name");
    else
        return push_error(ctx, "Expected {name:\"script.name\"}");

    bool is_arr = dukx_is_prop_truthy(ctx, -1, "array");

    std::string usi_error;

    unified_script_info usi = unified_script_loading(get_thread_id(ctx), script, usi_error);

    if(!is_arr)
    {
        if(usi.valid)
        {
            push_duk_val(ctx, usi.get_autogenerated_documentation(false));
        }
        else
        {
            push_duk_val(ctx, "No help available");
        }
    }
    else
    {
...
duk_ret_t msg__send(priv_context& priv_ctx, duk_context* ctx, int sl)
{
    ...
    if(found_ptr && get_caller_stack(ctx).size() > 0 && get_caller_stack(ctx)[0] == found_ptr->get_user_name())
        send_async_message(ctx, handle_client_poll_json(my_user).dump());
